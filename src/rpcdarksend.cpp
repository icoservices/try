...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <vvsaddress> <amount>\n"
            "vvsaddress, reset, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in VersatileVisioncoin.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in VersatileVisioncoin.conf\n"
                ...
                "  vote-many    - Vote on a VersatileVisionCoin initiative\n"
                "  vote         - Vote on a VersatileVisionCoin initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CVersatileVisionCoincoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a VersatileVisionCoin initiative (can be additionally filtered, partial match)\n"
