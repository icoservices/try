...
//
// Bootup the masternode, look for a 4000 vvs input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in VersatileVisioncoin.conf */
...
{
    CVersatileVisionCoincoinAddress address(collateralAddress);
