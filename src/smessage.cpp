...
int SecureMsgAddWalletAddresses()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& entry, pwalletMain->mapAddressBook)
    {
        ...
        CVersatileVisionCoincoinAddress coinAddress(entry.first);
...
int SecureMsgReadIni()
{
    ...
    while (fgets(cLine, 512, fp))
    {
        ...
        {
            smsgOptions.vvsanIncoming = (strcmp(pValue, "true") == 0) ? true : false;
        } else
...
int SecureMsgWriteIni()
{
    ...
    if (fprintf(fp, "newAddressRecv=%s\n", smsgOptions.fNewAddressRecv ? "true" : "false") < 0
        ...
        || fprintf(fp, "scanIncoming=%s\n", smsgOptions.vvsanIncoming ? "true" : "false") < 0)
    {
...
bool SecureMsgStart(bool fDontStart, bool vvsanChain)
{
    if (fDontStart)
    {
...
int SecureMsgScanMessage(uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, bool reportToGui)
{
    ...
    */
    ...
    for (std::vector<SecMsgAddress>::iterator it = smsgAddresses.begin(); it != smsgAddresses.end(); ++it)
    {
        ...
        CVersatileVisionCoincoinAddress coinAddress(it->sAddress);
...
int SecureMsgAddAddress(std::string& address, std::string& publicKey)
{
    ...
    */
    ...
    CVersatileVisionCoincoinAddress coinAddress(address);
    ...
    CVersatileVisionCoincoinAddress addressT(keyIDT);
...
int SecureMsgEncrypt(SecureMessage &smsg, const std::string &addressFrom, const std::string &addressTo, const std::string &message)
{
    ...
    */
    ...
    bool fSendAnonymous;
    CVersatileVisionCoincoinAddress coinAddrFrom;
    CKeyID ckidFrom;
    CKey keyFrom;
    ...
    CVersatileVisionCoincoinAddress coinAddrDest;
    CKeyID ckidDest;
    ...
    } else
    {
        ...
        vchPayload[0] = (static_cast<CVersatileVisionCoincoinAddress_B*>(&coinAddrFrom))->getVersion(); // vchPayload[0] = coinAddrDest.nVersion;
...
int SecureMsgSend(std::string &addressFrom, std::string &addressTo, std::string &message, std::string &sError)
{
    ...
    */
    ...
    CVersatileVisionCoincoinAddress coinAddrOutbox;
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, std::string)& entry, pwalletMain->mapAddressBook)
    {
        ...
        const CVersatileVisionCoincoinAddress& address = entry.first;
...
int SecureMsgDecrypt(bool fTestOnly, std::string &address, uint8_t *pHeader, uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
{
    ...
    */
    ...
    } else
    {
        ...
        CVersatileVisionCoincoinAddress coinAddrFromSig;
