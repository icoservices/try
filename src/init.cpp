...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: VersatileVisioncoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: VersatileVisioncoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizevvsamount=<n> " + _("Keep N VersatileVisionCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. VersatileVisionCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. VersatileVisionCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("VersatileVisionCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "VersatileVisionCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of VersatileVisionCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart VersatileVisionCoin to complete") << "\n";
    ...
    nAnonymizeVersatileVisionCoinAmount = GetArg("-anonymizevvsamount", 0);
    if(nAnonymizeVersatileVisionCoinAmount > 999999) nAnonymizeVersatileVisionCoinAmount = 999999;
    if(nAnonymizeVersatileVisionCoinAmount < 2) nAnonymizeVersatileVisionCoinAmount = 2;
    ...
    LogPrintf("Anonymize VersatileVisionCoin Amount %d\n", nAnonymizeVersatileVisionCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1vvs+1000 == (.1vvs+100)*10
       10vvs+10000 == (1vvs+1000)*10
    */
