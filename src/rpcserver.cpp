...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop VersatileVisionCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "VersatileVisionCoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use VersatileVisioncoind";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"VersatileVisionCoin Alert\" admin@foo.com\n"),
                strWhatAmI,
...
std::string HelpExampleCli(string methodname, string args){
    return "> VersatileVisioncoind " + methodname + " " + args + "\n";
}
